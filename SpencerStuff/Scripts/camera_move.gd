...
#Change cam pos back to MainCamPos
func ResetCamera():
	if main_cam_pos:
		var tween = create_tween()
		tween.tween_property(self, "global_position", main_cam_pos.global_position, move_time)
		print("Resetting camera to main position.")
	else:
