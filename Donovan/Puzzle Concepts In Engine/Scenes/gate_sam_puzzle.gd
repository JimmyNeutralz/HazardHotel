...
func _process(delta):
	...
	if (Input.is_action_just_pressed("raise_gate") or Global.check_array(4,4)) and not raised:
		if can_raise():
			raise_gate()
		else:
			print("Cannot raise gate yet — still electrified!")
			
	elif (Input.is_action_just_pressed("raise_gate") or Global.check_array(4,4)) and raised:
		if can_raise():
			lower_gate()
		else:
			print("Cannot raise gate yet — still electrified!")
